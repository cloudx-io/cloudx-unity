...
/*
 * Arbiter/TPA rewarded. Same as ArbiterInterstitialController plus the reward
 * callback of both SDKs surfaced through RewardEarned.
 */
public sealed class ArbiterRewardedController : ArbiterFullscreenController
{
    private const string AdFormatName = "rewarded";

    public event Action<CloudXArbiterPlatform, string> RewardEarned;

    private RewardedAd _adMobRewarded;

    public ArbiterRewardedController(
    ...
    protected override string AdMobAdSourceName() => AdSourceName(_adMobRewarded?.GetResponseInfo());
    ...
    protected override void AdMobLoad()
    {
        ...
        RewardedAd.Load(
            ...
            (ad, error) => MobileAdsEventExecutor.ExecuteInUpdate(() =>
            {
                ...
                {
                    OnAdMobLoadFailed(error?.GetMessage() ?? "AdMob returned no ad");
                    return;
                }
    ...
    protected override void AdMobShow()
    {
        _adMobRewarded.Show(reward => MobileAdsEventExecutor.ExecuteInUpdate(() =>
            RewardEarned?.Invoke(CloudXArbiterPlatform.AdMob, $"{reward.Amount} {reward.Type}")));
    }
    ...
    private void CloudXOnRewarded(CloudXAd ad, CloudXReward reward)
    {
        ...
        {
            RewardEarned?.Invoke(CloudXArbiterPlatform.CloudX, $"{reward.Amount} {reward.Label}");
        }
    }
    ...
    private void RegisterAdMobEvents(RewardedAd ad)
    {
        ...
        ad.OnAdPaid += adValue => MobileAdsEventExecutor.ExecuteInUpdate(() =>
            ReportAdMobPaidEvent(adValue, AdFormatName, AdSourceName(ad.GetResponseInfo())));
    }
}
